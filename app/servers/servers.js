        ...
        .then(function(tenant_token) {
          ...
          $http.get('http://192.168.122.183:8774/v2.1/' + tenant_id + '/servers')
            .then(function(response) {
              ...
              deferred.reject('serversFactory:list Error getting server list for tenant ' + tenant_id);
            });
        }, function(error) {
          error = error + '\nserversFactory:list Error getting tenant token';
  ...
  .controller('serversCtrl', function($scope, $routeParams, $window, serversFactory){
    ...
    serversFactory.list($routeParams.tenant_id)
      .then(function(data) {
        ...
        console.log(error + '\nserversCtrl - Error getting server list');
